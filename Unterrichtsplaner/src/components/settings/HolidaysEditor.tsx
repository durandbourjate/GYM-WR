...
export function HolidaysEditor({ holidays, onChange }: { holidays: HolidayConfig[]; onChange: (h: HolidayConfig[]) => void }) {
  ...
  return (
    <div className="space-y-1.5">
      <p className="text-[9px]" style={{ color: 'var(--text-muted)' }}>Ferienperioden als KW-Bereiche. Tagesauswahl erscheint automatisch bei Einzelwochen (z.B. Auffahrt).</p>
      {holidays.map(h => {
        ...
        <div key={h.id} className="rounded p-2 space-y-1.5" style={{ background: 'var(--bg-secondary)' }}>
          <div className="flex gap-1 items-center">
            ...
            <span className="text-[9px]" style={{ color: 'var(--text-muted)' }}>KW</span>
            ...
            {!isSingleWeek && !hasPartialDays && (
              <button onClick={() => toggleExpanded(h.id)}
                className="w-5 h-5 flex items-center justify-center text-[12px] cursor-pointer rounded"
                style={{ color: 'var(--text-dim)' }}
                title="Tagesauswahl anzeigen">
                {expandedDays.has(h.id) ? '▾' : '▸'}
              </button>
            )}
            <button onClick={() => remove(h.id)} className="ml-1 w-5 h-5 flex items-center justify-center text-[11px] text-red-400 cursor-pointer hover:bg-red-900/30 rounded" title="Entfernen">✕</button>
          </div>
            ...
            {['Mo', 'Di', 'Mi', 'Do', 'Fr'].map((day, di) => {
              ...
              return (
                <button key={day} onClick={() => {
                  ...
                  className={`px-1.5 py-px rounded text-[9px] cursor-pointer transition-all border ${
      ...
      <button onClick={addHoliday}
        className="w-full py-1 rounded border border-dashed text-[11px] cursor-pointer"
