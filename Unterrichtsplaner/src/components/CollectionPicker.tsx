...
export function CollectionPickerList({ onSelect, courseType }: {
  ...
  if (importable.length === 0) {
    return <div className="px-3 py-2 text-[9px] text-gray-500 italic text-center">Sammlung leer</div>;
  }

  return (<>{importable.map(item => (
    <button key={item.id} onClick={() => onSelect(item)}
      className="w-full px-3 py-1.5 text-left text-[10px] text-gray-300 hover:bg-slate-700 cursor-pointer">
