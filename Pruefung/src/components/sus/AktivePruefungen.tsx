 ...
 * Pollt alle 30 Sekunden nach neuen aktiven Prüfungen.
 */
export default function AktivePruefungen({ email }: AktivePruefungenProps) {
  ...
  // Einrichtungsprüfung/-übung ausblenden (Tutorial, nicht relevant)
  const relevante = pruefungen.filter(p =>
    !p.id.startsWith('einrichtung')
  )
      ...
      <div className="space-y-2">
        {relevante.map(p => (
          <a
            key={p.id}
            href={`?id=${p.id}`}
            className="block p-4 bg-white dark:bg-slate-800 rounded-xl shadow-sm border-2 border-green-200 dark:border-green-800 hover:border-green-400 dark:hover:border-green-600 hover:shadow-md transition-all"
          >
            <div className="flex items-center justify-between">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-lg">{p.modus === 'uebung' ? '📚' : '📝'}</span>
                  <span className="font-medium dark:text-white text-sm">{p.titel}</span>
                </div>
                <div className="flex items-center gap-2 text-xs text-slate-400">
                  <span>{p.modus === 'uebung' ? 'Übung' : 'Prüfung'}</span>
