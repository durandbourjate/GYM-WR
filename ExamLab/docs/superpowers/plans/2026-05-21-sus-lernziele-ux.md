...
>
> **Übernommen aus Task-7-Code-Review:** Beim Browser-E2E die AKTIVEN (selektierten) Schwierigkeits-/Fragetyp-Chips explizit prüfen — der `Chip` wurde auf einen `<span>`-Wrapper mit `overflow-hidden`/`rounded-full` umgebaut; auf eine mögliche Hairline-Naht an den runden Ecken (Border/Fill) achten, Light + Dark.
