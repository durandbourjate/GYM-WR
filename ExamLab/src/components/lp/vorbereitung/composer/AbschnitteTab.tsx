      ...
      <div className="flex items-center gap-2">
        <span className="flex-1 min-w-0 cursor-pointer" onClick={() => onEditFrage(frageId)} title="Klicken zum Bearbeiten">
          <span className="flex items-center gap-2 flex-wrap">
            ...
            {frage && fragenStats?.get(frage.id) && (() => { const perf = fragenStats.get(frage.id)!; const lq = perf.durchschnittLoesungsquote; const farbe = lq > 70 ? 'text-green-600 dark:text-green-400' : lq >= 40 ? 'text-amber-600 dark:text-amber-400' : 'text-red-600 dark:text-red-400'; return (<span className="inline-flex items-center gap-1"><span className={`text-[10px] font-medium ${farbe}`} title={`${perf.anzahlVerwendungen}× verwendet, ${perf.gesamtN} SuS`}>∅ {Math.round(lq)}%{perf.durchschnittTrennschaerfe != null && ` · TS ${perf.durchschnittTrennschaerfe.toFixed(2)}`}</span>{lq > 90 && <span className="text-[10px] text-amber-500 dark:text-amber-400 inline-flex items-center gap-0.5" title="Fast alle SuS lösen diese Frage richtig"><AlertTriangle className="w-3 h-3" aria-hidden="true" /> Sehr leicht</span>}{lq < 20 && <span className="text-[10px] text-red-500 dark:text-red-400 inline-flex items-center gap-0.5" title="Wenige SuS lösen diese Frage"><AlertTriangle className="w-3 h-3" aria-hidden="true" /> Sehr schwer</span>}{perf.durchschnittTrennschaerfe != null && perf.durchschnittTrennschaerfe < 0.2 && <span className="text-[10px] text-red-500 dark:text-red-400 inline-flex items-center gap-0.5" title="Frage trennt nicht zwischen starken und schwachen SuS"><AlertTriangle className="w-3 h-3" aria-hidden="true" /> Schlechte TS</span>}</span>) })()}
