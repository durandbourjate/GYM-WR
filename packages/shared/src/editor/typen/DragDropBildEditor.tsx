...
export default function DragDropBildEditor({ bildUrl, setBildUrl, zielzonen, setZielzonen, labels, setLabels }: Props) {
  ...
  const [labelsText, setLabelsText] = useState(safeLabels.join(', '))
  ...
  const handleBildKlick = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (!ersteEcke) {
      ...
      const neueZone: DragDropBildZielzone = {
        ...
        korrektesLabel: `Label ${safeZielzonen.length + 1}`,
      }
      setZielzonen(prev => [...prev, neueZone])
      setErsteEcke(null)
      setEditZone(neueZone.id)
    }
  }, [ersteEcke, safeZielzonen.length, setZielzonen])
  ...
  return (
    <div className="space-y-4">
      ...
      {safeZielzonen.length > 0 && safeLabels.length > 0 && (
        (() => {
          const fehlend = safeZielzonen.filter(z => !safeLabels.includes(z.korrektesLabel))
