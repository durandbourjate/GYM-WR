...
const SUB_DEFAULTS: { [K in FrageTyp]: Omit<Extract<Frage, { typ: K }>, keyof FrageBase | 'typ'> } = {
  ...
  pdf: {
    fragetext: 'Test',
    pdf: { typ: 'app', appPfad: 'test.pdf', mimeType: 'application/pdf', dateiname: 'test.pdf' },
    seitenAnzahl: 1,
    erlaubteWerkzeuge: [],
  ...
  hotspot: { fragetext: 'Test', bild: { typ: 'app', appPfad: 'test.svg', mimeType: 'image/svg+xml' }, bereiche: [], mehrfachauswahl: false } as Omit<HotspotFrage, keyof FrageBase | 'typ'>,
  bildbeschriftung: { fragetext: 'Test', bild: { typ: 'app', appPfad: 'test.svg', mimeType: 'image/svg+xml' }, beschriftungen: [] } as Omit<BildbeschriftungFrage, keyof FrageBase | 'typ'>,
  audio: { fragetext: 'Test', maxDauerSekunden: 60 } as Omit<AudioFrage, keyof FrageBase | 'typ'>,
  dragdrop_bild: { fragetext: 'Test', bild: { typ: 'app', appPfad: 'test.svg', mimeType: 'image/svg+xml' }, zielzonen: [], labels: [] } as Omit<DragDropBildFrage, keyof FrageBase | 'typ'>,
